...
tab1, tab2 = st.tabs(["Formulario", "Rel"])
with tab1:

    st.subheader("📐 Lançamentos - (Obras/Projetos)")
    ...
    with st.form("cadSolicitacao", clear_on_submit=True):
        with col1:
            ...
            times_desejados = ["Obras", "Projetos"]
            usuarios = [usr for usr in col_usuario.find({"time": {"$in": times_desejados}})]
